...
class Mdl_report_labarugi extends Mdl_core {
							...
							$similar='')
	{
		...
		"SELECT
			...
			SUM(
				CASE WHEN (j.tanggal::date BETWEEN '$tahun-$bulan-01' AND '$periode_akhir') AND j.nobukti LIKE '$kodejurnal%' AND dp.default_debet = 't' 
							THEN (CASE WHEN j.dk='D' THEN 1 ELSE -1 END) * j.rupiah 
			    	WHEN (j.tanggal::date BETWEEN '$tahun-$bulan-01' AND '$periode_akhir') AND j.nobukti LIKE '$kodejurnal%' AND dp.default_debet = 'f'  
					 		THEN (CASE WHEN j.dk='D' THEN 1 ELSE -1 END) *-1* j.rupiah
			...
			SUM( 
				CASE WHEN (j.tanggal::date BETWEEN '$tahun-$bulan-01' AND '$periode_akhir') AND j.nobukti NOT LIKE '$kodejurnal%' AND dp.default_debet = 't' 
				       	THEN (CASE WHEN j.dk='D' THEN 1 ELSE -1 END) * j.rupiah 
					WHEN (j.tanggal::date BETWEEN '$tahun-$bulan-01' AND '$periode_akhir') AND j.nobukti NOT LIKE '$kodejurnal%' AND dp.default_debet = 'f'
				      	THEN (CASE WHEN j.dk='D' THEN 1 ELSE -1 END) *-1* j.rupiah 
