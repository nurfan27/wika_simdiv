	...
	public function rincian()
	{
		...
		elseif(($type === 'rinci') and ($type2 == 'perkiraan'))
		{
			...
			while ( ($item = $saldo_tahunlalu->fetchObject()) !== false ) 
			{
				//$data['saldo']['kode'][]=$item->kduker;
				//$data['rincian']['kodes'][$j]=$item->kduker;
				$data['rincian'][substr($item->kdperkiraan,0,4)]['kdperkiraans'][$j]= $item->kdperkiraan;
				$data['saldo']['debet'][$item->kdperkiraan]=$item->total;
				$data['saldo']['kredit'][$item->kdperkiraan]=$item->kredit;
				$data['saldo'][substr($item->kdperkiraan,0,4)][$j]=$item->total;
				$data['saldo'][substr($item->kdperkiraan,0,2)][$j]=$item->total;
				//---kalo salah di delete
				$data['rincian']['kodegroup'][substr($item->kdperkiraan,0,2)][$j]=substr($item->kdperkiraan,0,4);
				$data['rincian']['kodegroup2'][$j]=substr($item->kdperkiraan,0,2);
				//---end
				
				$j++;
			}
			//var_dump($rincian_biayausaha->fetchObject());exit;
			while ( ($item = $rincian_biayausaha->fetchObject()) !== false ) 
			{
				$data['rincian']['kode'][$i]=$item->kduker;
				$data['rincian']['kodes'][$j]=$item->kduker;
				...
				$data['rincian'][$item->kduker]['nmperkiraan'][$i]= $item->nmperkiraan;
				$data['rincian']['nmperkiraan'][$item->kdperkiraan]= $item->nmperkiraan;
				$data['rincian'][substr($item->kdperkiraan,0,4).'-'.$item->kdperkiraan][$i]['nobukti']=$item->nobukti;
				$data['rincian'][substr($item->kdperkiraan,0,4).'-'.$item->kdperkiraan][$i]['tanggal']=$item->tanggal;
				$data['rincian'][substr($item->kdperkiraan,0,4).'-'.$item->kdperkiraan][$i]['keterangan']=$item->keterangan;
				$data['rincian'][substr($item->kdperkiraan,0,4).'-'.$item->kdperkiraan][$i]['rupiah']=$item->rupiah;			
				$data['rincian_total'][substr($item->kdperkiraan,0,4)][$i]=$item->rupiah;
				$data['rincian_total'][substr($item->kdperkiraan,0,2)][$i]=$item->rupiah;
				$data['rincian']['novi'][] = $item->namasingkatan;
				//---kalo salah di delete
				$data['rincian']['kodegroup'][substr($item->kdperkiraan,0,2)][$j]=substr($item->kdperkiraan,0,4);
				$data['rincian']['kodegroup2'][$j]=substr($item->kdperkiraan,0,2);
